...
**Flagged edges are not exported.** Any claim marked `needs_review` — an uncertain grounding
(parent-concept disease, multi-subunit/non-human target) or an unverified drug subject — is held
back from the KGX output and stays parked in the curated YAML until the flag is cleared. So the
graph contains only edges that are *not* flagged for review, while the knowledge base retains the
full curation record including the open questions. (Not flagged means the grounding was
confident, not that a human has verified the edge — `agent_type` records who produced it.)
